...
## Addendum — Creator-origin geometry-first undercarriage and perceived-mass animation (future phase)

**Creator's intent, edited for spelling and clarity:**

> “I wanted to find a simple, easy way to prototype this, so I thought: assuming that measurements from the outermost edge to the outermost edge would give us an impression of a model's perceived mass, we could do it very cheaply. We could then produce a logical model for the system to infer from. Alongside our bow-region analysis to determine shape, this could let us append a set of fun visual animations as accurately as possible to ships of varying sizes and shapes.”

The emphasis is on an **inexpensive, visually convincing heuristic**, not on reconstructing an uploaded spacecraft's true internal structure or claiming to have invented geometric mass estimation. This is the creator's proposed original combination and implementation direction; do not market it as the world's first without a separate prior-art review. “Bow-region analysis” here refers to the creator's proposed forward/bow shape interpretation; the precise axes and terminology should remain adjustable during prototyping rather than being treated as a confirmed existing algorithm.

### Intended experience and construction

Once the dirt landmass and its authoritative surface/collision sampling work, a small uploaded GLB spaceship should be able to descend, acquire a **new, automatically generated, custom-fitted underside base**, land with a satisfying thud and suspension compression, rebound or hop, and eventually transition to a **separate ground-driving engine**. Four wheels and their support/suspension assemblies belong to that appended base; the base is not a generic rectangular box that forces every uploaded ship into one silhouette. A long ship should receive an appropriately long, coherent base; a wide, uneven or asymmetrical one should have its outer geometry reflected in the base. The creator has not yet chosen the base's material/color (possibly silver or blue), so expose it as a replaceable style rather than fixing it in the default.

**Very cheap geometric heuristic to prototype first:** normalize the uploaded model's axes and transform; inspect its overall bounds and sample the **outermost-to-outermost spans** in selected longitudinal, lateral, vertical and diagonal directions. Use a small set of cross-sections, underside/bow-region samples, silhouette and coarse occupancy measurements to infer where its visibly larger masses seem concentrated and where a stable base might follow the outline. Generate a reduced, logical *perceived-mass map* and simple attachment/support lines. Use the inferred shape to build a fitted undercarriage and place four contact/wheel points with sensible clearance, rather than requiring expensive structural segmentation, mesh-by-mesh semantic recognition, neural inference or perfect closed-volume reconstruction. Handle disconnected meshes and decorative protrusions conservatively so a long antenna or thin wing does not incorrectly dominate the estimated body or wheel span. Supply manual adjustment/fallback when the shape is ambiguous.

The perceived-mass map is **not an actual mass, center-of-mass, density or engineering-strength measurement**: a GLB's exterior does not prove any of those. Estimate a *gameplay* mass distribution and center from the cheap geometric model, assign tunable vehicle mass/suspension ranges, and make each wheel's default loading, compression, body pitch/roll and rebound react coherently to the apparent major body regions. For a bulky end, favor a visibly deeper/heavier-feeling compression and rebound; for compact models, allow a quicker playful response. Use stable simulated contact and damping so “heavier bounce” means intentionally satisfying **visual and gameplay timing**, not automatically unrealistic uncontrolled acceleration or arbitrary size scaling. Provide creator overrides for the inferred mass, four attachment/contact positions, spring travel/stiffness, damping, rebound exaggeration and material styling.

**Fun, fitting animation is the reason for the inference:** from one cheap common inference/rigging step, procedurally generate ship-specific landing-gear deployment, wheel contact, hydraulic-style telescoping compression, chassis/body tilt, rebound, jump, settling, dust/thud and eventual driving reactions. The same four-wheel module should look sensible when fitted beneath different uploaded ships. Physical terrain samples and collision determine actual contact; the visual exaggeration is layered onto that coherent event and must not substitute an unrelated fake ground plane. The flight controller and later driving controller remain separate; hand over shared vehicle identity, pose and appropriate momentum explicitly. The optional wheel module must be removable/replaceable in a later running-avatar experience—no requirement that every avatar have wheels.

### Suggested tiny prototype and agent-facing observability

Start with three deliberately different models (compact, elongated, visibly front-heavy/asymmetrical), one inexpensive bounded geometry-analysis pass per uploaded model, one generated fitted base and four suspension/contact points. Show the sampled extremes/cross-sections, simplified perceived-mass map, inferred underside/bow shape, selected attachment lines, wheel positions, generated base outline and reason for each inference in a debug overlay/API; allow live manual corrections. Animate one authoritative-terrain landing and hop at several approach speeds and compare what works before adding full driving. Cache the inferred geometry profile by stable asset/version and recompute only after relevant asset/parameter changes, not every frame; keep the 4 GB hardware target in mind.

Expose later programmer/agent operations to inspect and explain the model analysis, diagnose bad attachment choices or misleading protrusions, preview base/suspension changes, adjust perceived-mass weighting and arcade animation parameters, and safely regenerate/undo the fitted base without modifying the source GLB, saved dirt ramps, or canonical geography. The future `API/Dirt/` is for terrain commands; a **separate, interoperable vehicle/rig API namespace** should own the undercarriage, with shared authoritative ground-height/contact queries. Be explicit that this is a **future follow-on proposal**, not completed by the dirt-landmass Codex request or the current terrain PR.

**Creator's principle:** Infer enough of an arbitrary ship's outer shape and visually suggested mass to generate its own believable undercarriage and enjoyable bounce—cheaply, quickly and without manual rigging. The exact appearance can be playful; it should still feel as though the fitted base and animations belong to *that particular uploaded ship*.
