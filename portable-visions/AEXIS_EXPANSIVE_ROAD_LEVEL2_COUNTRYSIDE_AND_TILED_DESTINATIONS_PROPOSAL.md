...
## Addendum — Four spatial forest layers with persistent sightlines (supersedes automatic 3↔4 alternation)

**Creator clarification:** Treat the forest's four layers as **ordered, geographically meaningful depth bands**, not a timer that arbitrarily alternates between rendering three and four layers. There may be three layers in view when the fourth is genuinely outside the camera's sightline, occluded or not yet needed, but **once a fourth layer appears it must stay present while its actual mapped geography remains visible**. A road turn, terrain occlusion, camera-direction change, or moving beyond the layer's real spatial extent may naturally take it out of view. Merely changing player speed, ticking a segment counter or switching a rendering budget must *not* make a visible distant forest or house abruptly vanish. When a fourth layer is no longer visible, the system may show only layers 1–3 until new far geography calls for another layer 4. Layer index describes a **relative presentation depth**; persistent assets retain stable identities and world/route positions as they advance from far to near.

### The four layers, nearest to farthest

1. **Layer 1 — Immediate roadside:** The road verge and nearby forest area, populated with creator-prescribed assets: trees (including interchangeable custom GLBs), bushes, ground detail, signs, event-capable objects and other authored/seeded features. Keep their original locations, scales, classifications and collision relevance consistent as the player approaches. Use full or simplified nearby geometry according to actual viewing distance and available rendering budget; do not repeat an identical roadside decoration arrangement on every road segment.
2. **Layer 2 — Behind the roadside:** A distinct adjoining belt of forest, grassland, desert or other selected biome, positioned *behind* layer 1 in actual scene depth. Use suitable lightweight representations and coherent parallax, and preserve any meaningful mapped landmarks, clearings, and road connections.
3. **Layer 3 — Farther countryside:** Distant forest fronts, canopy silhouettes, terrain/color/elevation shapes and occasional scenic landmarks corresponding to the expanded route's real modeled region. It should communicate forward road geometry and openings far enough ahead for the player's current controllable speed.
4. **Layer 4 — Farthest visible mapped area:** An optional farther forest boundary, hill, barn/house or other view-dependent mapped feature that extends the sense of landscape depth. If a far forest edge corresponds to this layer, align its position with that actual edge. The layer is not decorative wallpaper that may disappear when an abstract 3↔4 state flips.

These are **spatial depth layers for the near-ground forest experience**, distinct from the project's atmospheric altitude bands and the countryside's broader Level 2 representation policy. Layer 1 can be a belt along the road on both sides; layers 2–4 may occupy side and/or forward view according to actual camera angle and visible terrain. Do not force them to be four flat screen planes at fixed offsets independent of the world, and do not assume the same number of layers must always be visible.

### Stable visibility and promotion between levels of detail

Derive each layer from the shared route/forest geometry, anchored map/experience coordinates, deterministic placement rules, and persistent object IDs. The same house should not jump from “layer 4” to an unrelated house in “layer 3”; as the player approaches it may be *reclassified* from 4→3→2→1 and its renderer may change from a distant sprite to a closer sprite or the actual GLB, but its apparent position, silhouette/scale trajectory, identity and event/collision meaning remain coherent. Use overlap/crossfade and prefetch when needed. A very distant image may be grouped artwork instead of one proxy per tree, but its leading forest boundary and navigational openings must agree with the underlying expanded route.

A layer's continued visibility is decided by ordinary spatial tests: camera frustum, stable occlusion, angle, distance, geography and a hysteresis margin to prevent popping at thresholds. **Do not drop layer 4 because the code thinks “now use three.”** If a bend hides it, it may leave the frame naturally; when a new sightline opens, instantiate the next geographically relevant distant layer, not a random replacement that contradicts what was visible before. When the viewer turns back, the same far scenery should reappear at the same location. If memory requires cheaper rendering, reduce its resolution/complexity rather than making an unobstructed, already-visible far landmark vanish; provide an explicit bounded fallback policy for devices where a stable visual is impossible.

### Parallax, player speed and shared worlds

Foreground roadside assets move across the player's view most quickly, layer 2 more slowly, layer 3 more slowly still, and layer 4 according to its greater actual depth. Distances and visual passing rate may be artistically modulated by the Expansive Journey/optional speed mod, **but do not produce parallax that contradicts road turns, mapped obstacle distance or the player's time to react**. Keep navigational obstacles and shared multiplayer collision positions authoritative. Players may personalize cosmetic tree models, fog and far-layer shading in the same road session; a shared house, clearing or event-capable animal must still have a stable common location, or belong to an explicitly separate experience instance.

### Authoring and validation

The simple editor should label four ordered depth belts, permit each belt's biome profile, appearance assets, opacity/shading, parallax intensity, draw-distance/LOD and optional landmark/event slots to be changed with sliders and exact fields, and show a live preview from the selected driving camera. The programmer/agent API can inspect and edit the same versioned layer descriptors, determine why layer 4 is currently visible or hidden, and bind persistent objects to their shared spatial anchors. Avoid promising a permanently visible fourth layer when it is truly behind a hill or outside the field of view.

**Test:** Drive straight until a mapped barn/forest edge appears in layer 4; verify it stays in view as distance decreases, gradually resolves through layers 3, 2 and 1 without a pop or jump, and remains consistent when reversing. Test turns that genuinely remove it from the view and bring it back; changes in speed, 3-/4-layer visibility, night/fog lighting, custom tree swaps, camera selections, multiplayer cosmetic variants and constrained ~4 GB rendering. The earlier proposal's language about “alternating 3–4 layers” means the **number of spatial layers actually visible may vary as the scene warrants**—it must not mean arbitrary periodic deletion and replacement of the farthest image.

**Core principle:** A four-layer forest is a view onto one coherent expanded place. Layer 4 stays as long as the actual far landscape remains in sight; the forest's depth does not blink out just because an optimization counter changes.
