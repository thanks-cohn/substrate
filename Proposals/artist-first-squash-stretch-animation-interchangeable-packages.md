...
## Addendum B — ÆXIS / Toon Invasion: rig-first artwork and cinematic clash moves (2026-09-22)

**Status:** Proposed creator workflow and rendering/runtime contract; this addendum does not claim the engine, auto-fitting, move editor, or performance features are already implemented.

### B1. Artist-first, predictable puppet construction

Call the artist-facing character-and-move experience **Toon Invasion**, within the proposed **ÆXIS Engine** identity. Offer a visible library of example rigs and their **finished linked artwork**, with playable demonstrations showing idle, walk, turn, reach, squash, stretch, camera-angle limitations, and extreme poses. The creator can see what a given mechanical model reliably does *before* drawing for it. Offer beginner-to-complex templates (including elastic humanoids, oversized hands, nonhuman shapes, and modular limbs) without claiming one template can satisfy every anatomy or visual style.

A creator may select an already-rigged body part (such as a hand), resize/position it, pull or expand its polygonal covering, attach or draw art **on top of the covering**, and optionally modify the rig beneath it. The mechanical skeleton, deformable polygon surface, and artist-authored visible artwork are separate, independently editable responsibilities. Let the artist switch among **rig-only**, **polygon/attachment guides**, **art-only**, and **combined preview** without losing the pose or timing. Reusable rigged parts can be fitted to an artist's proportions; custom rigs remain possible. Preserve source art, per-view drawings, UV/attachment mappings, correction poses, and deformation limits. When the artist changes proportions, update or explicitly revalidate affected attachment and contact mappings rather than silently claiming automatic perfect skinning.

Keep the drawing workflow lightweight: show examples and expected outputs, let users wrap or draw over the predictable surfaces, then adjust only visibly problematic points, poses, or view-specific artwork. A character can start with a front-view approximation; extra views and artist-defined corrections improve turns and extreme deformation. The rig is a movement aid, not an obligation to make all poses physically realistic. Support local squash/stretch, nonuniform distortions, replacement illustrations, and hand-drawn smear/impact frames when a stretched texture alone does not preserve the intended design.

### B2. Cinematic clash: glow → optional vanishing → artist-directed move → return

Selected moves enter a **short, authored cinematic capsule**, not an unexplained abrupt camera cut. On activation **both participating fighters glow**, signaling that they are entering a shared special interaction. For a suitably fast or fantastical move, both may **vanish from ordinary view** in a bright flash, trail, or silhouette: to the player, the exchange was too fast to follow. The engine then switches to the initiating artist's prepared camera angle or camera pan and plays the corresponding rig poses, toon artwork, and timed animation. At an authored impact beat, the struck character may turn white / flash as the saved impact drawing or short hand-drawn sequence appears. The figures then reappear in the ordinary world, the regular camera is restored, and gameplay continues from the resolved positions and states.

Let the move creator direct only the necessary small cinematic section: entry pose, camera cut/path, frame/timeline markers, rig motion, artwork overlays or view-specific replacements, contact/impact markers, white-flash treatment, recovery, and exit/return. Include a timeline scrubber showing precisely what is visible at each frame. The glow/vanishing motif and artistic cut-ins are configurable presentation tools; they must not be required for every mundane action or forced onto every creator's art style. Camera locking is **action-scoped**, not a global loss of player camera control.

### B3. Counter-moves inherit the original artist's camera contract

The initiating move publishes opt-in **counter windows** and reusable interaction surfaces/anchors (e.g., fist, forearm, wrist, shield edge, shoulder, impact plane), plus the applicable **camera pan, composition, framing, and time/frame interval**. A counter artist previews the initiating move, sees its rig/surface references and camera at the actual frames, and draws or attaches their own counter artwork to compatible poses and polygons. Their counter must fit the original artist's provided camera path over the declared shared interval, instead of silently replacing the shot with an unrelated camera. This gives independent artists predictable shared frames on which to build a block, catch, reversal, throw, or custom art burst.

At the counter window, a deterministic gameplay condition selects the ordinary hit branch or a compatible counter branch. The counter supplies its own animation, art overlays, contact mapping, outcome and valid exit state; if it needs additional duration, use an explicit authorized extension/transition after the shared camera interval. Validate reference-space and timing compatibility in the editor and offer a fallback when a counter package is incompatible or missing. The originating artist controls which windows, anchors, and camera intervals are exposed; the counter artist controls their own responding character's art and permitted response. Do not require the original creator to pre-draw each third-party counter.

### B4. Performance: let the visual transition create a lighter work interval

The glow/vanish/camera-switch may provide an **opportunity** to reduce expensive full-scene work during intense multi-fighter scenes. Where safe, stop drawing occluded or off-camera battlefield content, suspend or lower update rates for nonessential environmental effects and remote actors, and render only the bounded cinematic stage and required characters/art. Preload or cache likely camera and artwork assets before entry; stream/defer nonessential work during the controlled shot; offer reduced-resolution effects or simple silhouette/fade fallbacks for low-memory devices. This is **not** a guaranteed CPU/GPU rest period: a complex cinematic can cost more than ordinary gameplay, and transition/preload spikes must be profiled.

Maintain authoritative gameplay time, collisions, combat outcomes, input policy, and multiplayer/network synchronization even if visual-only background simulation is paused. Restore the ordinary camera, render resources, world state, character transforms, and visibility consistently after **success, counter, interruption, cancellation, asset failure, or reduced-motion fallback**; prevent snapping or stale hitboxes at the cinematic exit. Keep a safe direct-cut/no-vanish option for accessibility or a missed performance budget.

### B5. Portable move package and acceptance demonstration

Define an engine-neutral, versioned move package linking (without duplicating) the underlying rigs, artwork, and clips. Suggested fields: \`moveId\`, \`rigCompatibility\`, \`participants\`, \`entryState\`, \`glowCue\`, \`optionalVanishCue\`, \`cameraTrack\`, \`frameMarkers\`, \`interactionAnchors\`, \`counterWindows\`, \`impactFlash\`, \`artOverrides\`, \`animationClips\`, \`outcomeBranches\`, \`exitStates\`, \`performanceBudget\`, and \`fallback\`. Use explicit coordinate spaces, time units, asset references/provenance, and permissions; do not grant a move package privileged code execution. Provide an API/inspection surface so authorized tools and agents can discover templates, camera intervals, compatible counter surfaces, and declared asset requirements. A renderer adapter may implement the presentation without owning the canonical gameplay state.

**Acceptance demonstration:** On a modest device, choose an example elastic humanoid with a pre-rigged oversized hand, pull its polygon covering to fit, draw or import toon art on top, toggle rig/art views, and preview squash/stretch. Create one special attack: both characters glow, optionally vanish, the camera follows the artist's saved pan, the victim flashes white as saved impact art appears, and both reappear in their resolved normal-world positions. Then import a second artist's compatible counter that uses the **same provided camera pan and published contact frame**, branches at the declared counter window, and returns to gameplay with its own validated outcome. Profile scene load during the transition, verify resource/input/state restoration and network-safe outcomes, and demonstrate an inexpensive fallback. Preview and save locally without publishing or overwriting the original rig/art.
